...
class MainClass {
  public static void Main (string[] args) {
    ...
    Console.WriteLine( CheckPassword(String.Empty) == false );        // 1
    Console.WriteLine( CheckPassword("Abcdefghijk\\") == true );      // 5: special characters
    Console.WriteLine( CheckPassword(@"Abcdefghijk""") == true );     // 5
    Console.WriteLine( CheckPassword(@"Abcdefghijk\") == true );      // 5
  }
  ...
  public static bool CheckPassword(string password) {
    ...
    char[] specialCharacters = @"!@#$%^&*()?/>.<,:;'\|}]{[_~`+=""-".ToCharArray();    // Verbatim 방식
